<script lang="ts">
    import flower1 from '$lib/assets/flower1-dark.png'
    import flower2 from '$lib/assets/flower2-dark.png'
    import flower3 from '$lib/assets/flower3-dark.png'
</script>

<div class="entrypoint">
    <div class="entrypoint-content">
        <h1>Hi, I'm Emma Xing.</h1>
        <em><p>Currently working as a Member of Technical Staff at TextQL</p></em>
        <em><p>& studying CS at the University of Waterloo.</p></em>
        <p>I’m a software engineer interested in cultivating authenticity.</p>
        <p>I’m passionate about building community and leveraging tech for good.</p>
        <div>
        <em><p>If this resonates with you, let's connect!</p></em>
            <a href="mailto:emma.xing@uwaterloo.ca" target="_blank" rel="noopener noreferrer">emma.xing@uwaterloo.ca</a>
            <a href="https://www.linkedin.com/in/emmaxing/" target="_blank" rel="noopener noreferrer">LinkedIn</a>
            <a href="https://github.com/emma-x1" target="_blank" rel="noopener noreferrer">GitHub</a>
            <a href="https://x.com/emm4x3" target="_blank" rel="noopener noreferrer">X</a>
        </div>
    </div>
</div>

<style>
    .entrypoint {
        width: 100%;
        height: 80vh;

        display: flex;
        justify-content: center;
        align-items: center;
        flex-direction: column;
        position: relative;
    }

    .entrypoint-content {
        text-align: center;
        max-width: 40rem;
        padding: 2rem;
        gap: 1rem;
    }

    .entrypoint-content a {
        margin-right: 1rem;
        display: inline-block;
    }

</style>